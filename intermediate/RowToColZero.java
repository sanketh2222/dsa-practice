import java.io.File;
import java.io.IOException;
import java.util.Arrays;

public class RowToColZero {

    public int[][] solveOld(int[][] A) {
        int rowSize = A.length, colSize = A[0].length;
        int[][] result = Arrays.copyOf(A, colSize);
        for (int i = 0; i < rowSize; i++) {
            for (int j = 0; j < colSize; j++) {
                if (A[i][j] == 0) {
                    int[] temp = new int[colSize];
                    //replace row with 0 row 
                    result[i] = temp;
                    //replace col with 0 col 
                    for (int k = 0; k < rowSize; k++) {
                        result[k][j] = 0;
                    }
                }
            }
        }

        return result;
    }

    public int[][] solve(int[][] A) {
        int rowSize = A.length, colSize = A[0].length;
        int result[][] = new int[rowSize][colSize];

        for (int i = 0; i < rowSize; i++){
            result[i] = Arrays.copyOf(A[i], A[i].length);
        }
        for (int i = 0; i < rowSize; i++) {
            for (int j = 0; j < colSize; j++) {
                if (A[i][j] == 0) {
                    int[] temp = new int[colSize];
                    int col = j;
                    result[i] = temp;
                    for (int k = 0; k < rowSize; k++) {
                        result[k][col] = 0;
                    }
                }
            }
        }

        return result;
    }

    public static void main(String[] args) throws IOException {
        RowToColZero colZero = new RowToColZero();
        int[][] matrix = {
                { 97, 18, 55, 1, 50, 17, 16, 0, 22, 14 },
                { 58, 14, 75, 54, 11, 23, 54, 95, 33, 23 },
                { 73, 11, 2, 80, 6, 43, 67, 82, 73, 4 },
                { 61, 22, 23, 68, 23, 73, 85, 91, 25, 7 },
                { 6, 83, 22, 81, 89, 85, 56, 43, 32, 89 },

                { 0, 6, 1, 69, 86, 7, 64, 5, 90, 37 },
                { 10, 3, 11, 33, 71, 86, 6, 56, 78, 31 },
                { 16, 36, 66, 90, 17, 55, 27, 26, 99, 59 },
                { 67, 18, 65, 68, 87, 3, 28, 52, 9, 70 },
                { 41, 19, 73, 5, 52, 96, 91, 10, 52, 21 }
        };

        colZero.solve(matrix);

        File file = new File("file.txt");
        file.createNewFile();  
    }
}
